...
if not dff.empty:
    ...
    with kpi_col2:
        ...
        <div class="card-box" style="text-align: center;">
            ...
            <div style="font-size: 17px; font-weight: 700; color: #111; margin-top: 20px;">{fuera_meta} de {vehiculos_total} vehicles</div>
    ...
    # 1. Columna Grupo (Líneas cortas anti-recorte)
    with mid_col1:
        g_df = dff.groupby("grupo")
        g_df = g_df.agg({"ralenti_seg": "sum", "encendido_seg": "sum"})
        grupo_df = g_df.reset_index()
    ...
    # 2. Columna Tipo de Vehículo (Líneas cortas anti-recorte)
    with mid_col2:
        ...
        if "tipo_vehiculo" in dff.columns and not dff["tipo_vehiculo"].isna().all():
            t_df = dff.groupby("tipo_vehiculo")
            t_df = t_df.agg({"ralenti_seg": "sum", "encendido_seg": "sum"})
            tipo_df = t_df.reset_index()
    ...
    # 3. Columna Top 5 Ranking Tabla (Líneas cortas anti-recorte)
    with mid_col3:
        top_df = dff.groupby("nombre_dispositivo")
        top_df = top_df.agg({"ralenti_seg": "sum", "encendido_seg": "sum"})
        top = top_df.reset_index()
    ...
    if not dff_ultimo_mes.empty:
        evo_df = dff_ultimo_mes.groupby("fecha")
        evo_df = evo_df.agg({"ralenti_seg": "sum", "encendido_seg": "sum"})
        evo = evo_df.reset_index()
